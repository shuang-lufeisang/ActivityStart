...
import com.duan.android.activitystartup.util.cache.ACache;
                        ...
                        .doOnNext(new Action1<T>() {
                            ...
                            public void call(final T data) {
                                Schedulers.io().createWorker().schedule(new Action0() {
                                    @Override
                                    public void call() {
                                        LogUtils.printInfo(TAG,"get data from network finish ,start cache...");
                                        ...
                                        for (Field field : fields) {
                                            ...
                                            if (className.equalsIgnoreCase("List")) {
                                                try {
                                                    ...
                                                    LogUtils.printInfo(TAG,"list==" + list);
                                                    if (!list.isEmpty()) {
                                                        ACache.get(Utils.getApp()).put(key, new Gson().toJson(data, clazz));
                                                        LogUtils.printInfo(TAG,"cache finish");
                                                    }
    ...
    public static <T> Observable rxCreateDiskObservable(final String key, final Class<T> clazz) {
        return Observable.create(new Observable.OnSubscribe<String>() {
            ...
            public void call(Subscriber<? super String> subscriber) {
                LogUtils.printInfo(TAG,"get data from disk: key==" + key);
                String json = ACache.get(Utils.getApp()).getAsString(key);
                LogUtils.printInfo(TAG,"get data from disk finish , json==" + json);
